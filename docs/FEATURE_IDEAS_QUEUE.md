...
### [IDEA-2026-05-17-23] PC-control: aspect-ratio-matched screenshot resolution

- **Source / context:** Read farzaa/clicky's `ElementLocationDetector.swift` 2026-05-17. It never resizes a screenshot to one fixed size — it picks the Anthropic-recommended resolution closest to the display's actual aspect ratio (`1024×768` 4:3, `1280×800` 16:10, `1366×768` ~16:9). Its comment: squishing a 16:10 display into 4:3 distorts the image the model sees and *"degrades X-axis coordinate accuracy."* AI Computer's `_capture_screenshot_b64` hard-caps at 1280×800 regardless of the real ratio.
- **Why it fits Ai_computer:** Off-target clicks were a top finding in the PC-control audit (IDEA-17-05). A distorted screenshot is a root cause — the model's coordinates are computed against a stretched image. Small fix, real accuracy gain for every desktop/computer-use task.
...
- **Source / context:** Read farzaa/clicky's `CompanionManager.swift` 2026-05-17. The model embeds a pointing tag directly in its normal reply — `[POINT:1100,42:color inspector]` (x,y in screenshot coords + a short label), or `[POINT:none]` when pointing wouldn't help. The reply text has the tag stripped before it's shown/spoken; a parser extracts the coordinate. It is model-agnostic — works with ANY vision model, no Anthropic Computer Use dependency.
- **Why it fits Ai_computer:** Gives the *model* a way to say "here's the thing I'm talking about" — for free. It upgrades two features just shipped: (a) explain mode (IDEA-17-22) can point at the element it describes; (b) the pointer overlay (IDEA-17-21) becomes driven by the model's intent, not just raw click coordinates.
- **Scope (this PR only):** (1) Add the `[POINT:x,y:label]` / `[POINT:none]` convention to the explain-mode system prompt in `app/agent.py`. (2) Parse the tag out of the reply: strip it from the displayed/spoken text, and if a coordinate is present and the pointer overlay is enabled, call `_flash_pointer` (from IDEA-17-21) at that point. (3) Coordinates are in screenshot space — scale to real screen coords before flashing. ~70-100 LOC + tests for the parser (tag present / `none` / absent / malformed).
...
### [IDEA-2026-05-19-09] Model governance: provider + model blocklists (enterprise parity)

- **Source / context:** Competitor research 2026-05-19 — Cursor 3.3 offers provider-level and model-level allow/blocklists for enterprise admins; OpenHands shows model names on cards. AI_computer's ALLOWED_MODELS is coarse (all-or-nothing). Enterprise customers need granular control: \"block OpenAI\", \"block all models < 32K context\", \"prefer local Ollama over OpenRouter\".
- **Why it fits Ai_computer:** Enterprise adoption is blocked by lack of cost controls and model governance. Adding `BLOCKED_PROVIDERS` (e.g., \"openai,anthropic\") and `BLOCKED_MODELS` (e.g., \"gpt-4o,claude-opus\") + per-task model selection UI unlocks enterprise sales and trust. Pairs with provider_info SSE events (already shipped).
- **Scope (this PR only):** (1) Add env vars: `BLOCKED_PROVIDERS` (frozenset), `BLOCKED_MODELS` (frozenset), `PREFERRED_PROVIDER` (fallback order hint). (2) Update `_openrouter_models_to_try()` to filter both provider and model name. (3) Add flags to composer UI: \"Model: [select] / Provider: [select]\" dropdown to override defaults per-task. (4) Validate at init: if all models are blocked, raise ValueError with friendly message. ~60-80 LOC + tests.\n- **Acceptance criteria:** Env vars block specified providers/models; blocked models are skipped during fallback. UI dropdowns let users override per-task. Suite green; manual test covers blocked fallback (first model blocked → use second).\n- **Out of scope:** Cost limits; spend tracking; per-team quotas (separate IDEA).\n- **Priority:** MEDIUM.\n- **Status:** queued

### [IDEA-2026-05-19-10] Thinking/reasoning budget UI toggle + cost badge

- **Source / context:** Competitor research 2026-05-19 — Aider's `/think-tokens` (8k, 10.5k, 0.5M) and `/reasoning-effort` (low/med/high/max) expose reasoning as a user-facing cost/quality tradeoff. Claude Opus 4.7's adaptive thinking is invisible; opusplan (Opus plan + Sonnet execute) is a hybrid. AI_computer has no reasoning budget UI; it's all-or-nothing based on model.
- **Why it fits Ai_computer:** Free models (Gemma, Llama on OpenRouter) take 8-21s per call. Thinking tokens add 50% overhead. Users should see \"reasoning: high (extra 5-10s)\" vs \"reasoning: off (saves 10s)\" and choose. Pairs with IDEA-19-04 (streaming reasoning), making the slowness transparent and controllable. High impact on perceived UX for free-tier users.\n- **Scope (this PR only):** (1) Add composer dropdown: \"Reasoning: [Off / Low / Medium / High]\" (maps to think_budget_tokens in provider request). (2) If model doesn't support thinking, gray out and show tooltip \"Model does not support extended thinking\". (3) On task card, show badge \"[reasoning: High]\" so user knows why a task took 30s. (4) Pass reasoning level to all provider calls; update SSE to emit thinking token counts (if available). ~100-140 LOC + tests.\n- **Acceptance criteria:** Dropdown controls reasoning budget; task cards show reasoning badge; thinking tokens appear in SSE stream. Non-thinking models gracefully skip. Pytest green; manual test on Claude Opus (supports thinking) and Gemma (doesn't).\n- **Out of scope:** Auto-tuning reasoning per task complexity; reasoning budgets per model tier.\n- **Priority:** MEDIUM (pairs with IDEA-19-04; defer until that ships).\n- **Status:** queued
