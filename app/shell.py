"""Native always-on-top floating shell for the AI Computer voice widget.
...
def run(port: int, manage_server: bool, width: int, height: int) -> int:
    try:
        ...
        print(
            "[AI Computer Shell] pywebview is not installed.\n"
    ...
    if manage_server:
        ...
        if not _wait_for_server(f"http://127.0.0.1:{port}/healthz", timeout=1.0):
            ...
            if not _wait_for_server(f"http://127.0.0.1:{port}/healthz", timeout=30.0):
                print(
                    f"[AI Computer Shell] Server on 127.0.0.1:{port} did not "
        ...
        if not _wait_for_server(f"http://127.0.0.1:{port}/healthz", timeout=2.0):
            print(
                f"[AI Computer Shell] No server answering on 127.0.0.1:{port}. "
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        prog="app.shell",
        description="Launch the AI Computer voice widget as an always-on-top floating window.",
    )
